...
from PyTernary import PyTernary 
import sys
...
class mainWindow(QtGui.QMainWindow):
    ...
    def create_main_frame(self):
        ...
        self.PyTernary = PyTernary(self.fig, short_labels=self.shortTitles)
        ...
        self.connect(self.btn_editGroups, QtCore.SIGNAL('clicked()'),
                     lambda: PlotSettingsWindow(self, self.PyTernary,
                                                self.canvas).exec_())
    ...
    def removeCurrentTab(self):
        '''Remove current tab.'''
        self.removeTab(self.tab.currentIndex())
    ...
    def removeGroupTab(self, index):
        ...
        self.PyTernary.remove_plot(index)
    ...
    def addGroupTab(self):
        ...
        self.PyTernary.add_null_plot(label=groupName)
    ...
    def __changeHorizontalHeader(self, index):
        ...
        if ok:
            ...
            self.PyTernary.set_short_labels(self.shortTitles)
            self.canvas.draw()
    ...
    def __updateGroupPlot(self, index):
        ...
        if data:
            self.PyTernary.update_plot(index, data)
            self.canvas.draw()
...
class PlotSettingsWindow(QtGui.QDialog):
    def __init__(self, parent, PyTernary, canvas):
        '''Constructor. 
        plots: plot data
    ...
    def __addElementsTable(self):
        ...
        self.chkBoxPlot = []
        self.chkBoxLegend = []
        for row in range(self.PyTernary.get_n()):
    ...
    def __setNewGroupName(self, row, col):
        if col == 0:
            ...
            if newGroupName:
                self.PyTernary.change_properties(row, label = newGroupName)
                self.parent.tab.setItemText(row, newGroupName)

    def __changeCheckBoxesColumn(self, index):
        if index == 4:
            for i in range(self.PyTernary.get_n()):
                chkValue = not self.PyTernary.get_plot_visibility(i)
                self.PyTernary.set_plot_visibility(i, chkValue)
                self.chkBoxPlot[i].setCheckState(QtCore.Qt.Checked if chkValue else QtCore.Qt.Unchecked)
        if index == 5:
            for i in range(self.PyTernary.get_n()):
                chkValue = not self.PyTernary.get_legend_visibiliy(i)
                self.PyTernary.set_legend_visibility(i, chkValue)
    ...
    def __setCellColor(self, row, col):
        if col==1:
            ...
            if color.isValid():
                self.PyTernary.change_properties(row, color=color.name())
    ...
    def __addRow2ElementsTable(self, row):
        ...
        #Label
        item = QtGui.QTableWidgetItem(self.PyTernary.get_properties(row, 'label'))
        table.setItem(row, 0, item)

        #Color
        color = QtGui.QColor(self.PyTernary.get_properties(row, 'color'))
...
#        table.cellClicked.connect(lambda x,y: self.setCellColor(x, y, frm))
        ...
        marker = self.PyTernary.get_properties(row)
        ...
        #Marker size
        sbox = QtGui.QSpinBox()
        d_markersize = self.PyTernary.get_properties(row)
        ...
        #Plot
        chkValue = self.PyTernary.get_plot_visibility(row)
        frame = QtGui.QFrame()
        chkBoxPlot = QtGui.QCheckBox('')
        chkBoxPlot.setCheckState(QtCore.Qt.Checked if chkValue else QtCore.Qt.Unchecked)
        chkBoxPlot.clicked.connect(lambda x: self.PyTernary.set_plot_visibility(row, x))
        ...
        #Legend
        frame = QtGui.QFrame()
        chkValue = self.PyTernary.get_legend_visibility(row)
        chkBoxLegend = QtGui.QCheckBox('')
        chkBoxLegend.setCheckState(QtCore.Qt.Checked if chkValue else QtCore.Qt.Unchecked)
        chkBoxLegend.clicked.connect(lambda x: self.PyTernary.set_legend_visibility(row, x))
